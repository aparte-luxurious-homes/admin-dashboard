...
export function RotateOutboundUrl() {
  ...
  return useMutation({
    mutationFn: (unitId: string | number) =>
      axiosRequest.post(API_ROUTES.ical.units.outboundRotate(unitId)),
    onSuccess: (_, unitId) => {
...
export function GetExternalBookings(
  unitId: string | number,
  ...
  return useQuery({
    queryKey: [IcalRequestKeys.externalBookings, unitId],
    queryFn: () =>
      axiosRequest.get(API_ROUTES.ical.units.externalBookings(unitId)),
    refetchOnWindowFocus: true,
    enabled,
  });
}
...
export function ForcePollFeed() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (feedId: string | number) =>
      axiosRequest.post(API_ROUTES.ical.admin.forcePoll(feedId)),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [IcalRequestKeys.platformFeeds],
      });
    },
  });
}
...
export function DisableFeed() {
  ...
  return useMutation({
    mutationFn: (feedId: string | number) =>
      axiosRequest.patch(API_ROUTES.ical.admin.disable(feedId)),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [IcalRequestKeys.platformFeeds],
      });
...
export function EnableFeed() {
  ...
  return useMutation({
    mutationFn: (feedId: string | number) =>
      axiosRequest.patch(API_ROUTES.ical.admin.enable(feedId)),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [IcalRequestKeys.platformFeeds],
      });
...
export function ResolveConflict() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (conflictId: string | number) =>
      axiosRequest.post(API_ROUTES.ical.admin.resolveConflict, {
