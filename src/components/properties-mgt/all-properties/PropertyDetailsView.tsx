...
export default function PropertyDetailsView({
  ...
  const { mutate: updateBookingMode, isPending: bookingModeUpdating } =
    UpdateBookingMode();
  const { mutate: reassignOwner, isPending: reassignOwnerLoading } =
    ReassignPropertyOwner(propertyId);
  ...
  const { data: ownersList, isLoading: ownersLoading } = GetAllUsers(
    1,
    12,
    ownerSearchTerm,
    UserRole.OWNER,
  ...
  const [averageRating, setAverageRating] = useState<number>(
    property?.meta?.total_reviews
      ? property?.meta?.total_rating / property?.meta?.total_reviews
      : 0,
  );
  ...
  const [selectedDoc, setSelectedDoc] = useState<IPropertyDocument | null>(
    null,
  );
  ...
  useEffect(() => {
    if (data) {
      ...
      setAverageRating(
        property?.meta?.total_reviews
          ? property?.meta?.total_reviews / property?.meta?.average_rating
          : 0,
      );
    }
          ...
          <div className="divide-y divide-zinc-100">
                  ...
                  <>
                            ...
                            >
                              <div className="p-2 bg-zinc-50 rounded-lg group-hover:bg-primary/10 transition-colors">
                                {el.name === "AIR CONDITIONER" ? (
                                  <TbAirConditioning className="text-xl text-zinc-500 group-hover:text-primary" />
                                ) : el.name === "HOT TUB" ? (
                                  <PiBathtub className="text-xl text-zinc-500 group-hover:text-primary" />
                                ) : el.name === "Wi-FI" ? (
                                  <IoWifi className="text-xl text-zinc-500 group-hover:text-primary" />
                                ) : el.name === "PS5" ? (
                                  <IoGameControllerOutline className="text-xl text-zinc-500 group-hover:text-primary" />
                                ) : el?.name === "TV" ? (
                                  <FaTv className="text-xl text-zinc-500 group-hover:text-primary" />
                                ) : (
                                  <FaSwimmer className="text-xl text-zinc-500 group-hover:text-primary" />
                                )}
                              </div>
                              <span className="text-[10px] sm:text-xs font-semibold text-zinc-600 text-center leading-tight">
                                {el.name}
                              </span>
                            </div>
                          ))}
                      ...
                      {property?.documents?.length > 0 ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {property.documents.map(
                            (doc: IPropertyDocument, index: number) => (
                              <div
                                key={index}
                                ...
                                <div className="flex items-center gap-3">
                                  <div className="p-2 bg-zinc-50 rounded-lg group-hover:bg-primary/10 transition-colors flex-shrink-0">
                                    <Icon
                                      ...
                                      className="text-zinc-400 group-hover:text-primary text-lg"
                                    />
                                  </div>
                                  <div className="flex-1 min-w-0">
                        ...
                        <EmptyState
                          icon="solar:folder-favourite-bold-duotone"
                    ...
                    <section>
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <Link
                          href={PAGE_ROUTES.dashboard.propertyManagement.allProperties.units.create(
                            property?.id,
                          )}
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-primary/10 text-primary text-xs font-bold rounded-xl hover:bg-primary hover:text-white transition-all"
                        >
                            ...
                            >
                              <div className="relative h-36 sm:h-44 bg-zinc-100 overflow-hidden">
                                {el.media && el.media.length > 0 ? (
                                  (() => {
                                    const firstImage = el.media.find(
                                      (m: any) =>
                                    ...
                                    const thumbSrc =
                                      ...
                                      "/png/placeholder.png";
                                    const thumbIsVideo =
                                      (thumb.media_type || thumb.mediaType) ===
                                      "VIDEO";
                                    return thumbIsVideo ? (
                                      <video
                                        src={thumbSrc}
                              ...
                              <div className="p-3 sm:p-4 flex flex-col gap-1.5">
                                ...
                                <p className="text-xs text-zinc-400 line-clamp-2 leading-relaxed">
                                  {el.description ||
                                    "View this unit for more details."}
                                </p>
                      ...
                      <div className="flex items-start gap-3 mb-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-amber-700 mt-0.5 leading-relaxed">
                            This property is not visible to the public yet.
                          </p>
                        </div>
                      </div>
                      {property?.agent ? (
                        <button
                          onClick={() => {
                            ...
                            if (verificationId) {
                              ...
                              router.push(
                                PAGE_ROUTES.dashboard.propertyManagement.allProperties.verifications.base(
                                  propertyId,
                                ),
                              );
                            }
                ...
                <div className="bg-white border border-zinc-100 rounded-2xl p-4 sm:p-5 shadow-sm">
                  ...
                  <div className="space-y-3">
                    <OverviewRow
                      ...
                      value={
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${property?.isVerified || property?.is_verified ? "bg-teal-100 text-teal-700" : "bg-red-50 text-red-600"}`}
                        >
                          {property?.isVerified || property?.is_verified
                            ? "Verified"
                            : "Unverified"}
                    ...
                    <OverviewRow
                      icon={
                        <PiBuildingApartment className="text-zinc-400 text-sm" />
                      }
                      label="Inventory"
                      value={
                    ...
                    <OverviewRow
                      icon={
                        <RiBuilding2Line className="text-zinc-400 text-sm" />
                      }
                      label="Type"
                      value={
                  ...
                  {!editMode && (
                    <div className="mt-5 pt-4 border-t border-zinc-50 space-y-2">
                      ...
                      <Link
                        href={PAGE_ROUTES.dashboard.propertyManagement.allProperties.verifications.base(
                          propertyId,
                        )}
                  ...
                  <div className="space-y-4">
                        ...
                        <PersonRow
                          image={
                            (property?.owner?.profile?.profileImage ||
                              property?.owner?.profile?.profile_image) ??
                    ...
                    {property?.agent && user?.role !== UserRole.AGENT ? (
                      <div>
                        ...
                        <PersonRow
                          image={
                            (property?.agent?.profile?.profileImage ||
                              property?.agent?.profile?.profile_image) ??
                            "/png/sample_profile.png"
                          }
                          name={
                            property?.agent?.profile?.firstName
                    ...
                    <div className="bg-white border border-zinc-100 rounded-2xl p-4 sm:p-5 shadow-sm">
                      <div className="pb-3 border-b border-zinc-50 mb-1">
                        ...
                        <p className="text-[11px] text-zinc-400 mt-1.5">
                          Choose how guests book your property.
                        </p>
                      </div>
                      <div className="flex flex-col gap-2.5 mt-4">
                        ...
                        <BookingModeButton
                          ...
                          icon={
                            <Icon
                              ...
                              className={`text-base ${(property?.bookingMode ?? property?.booking_mode) === BookingMode.INSTANT || !(property?.bookingMode ?? property?.booking_mode) ? "text-primary" : "text-zinc-400"}`}
                            />
                          }
                          iconBg={
                            (property?.bookingMode ??
                        ...
                        <BookingModeButton
                          ...
                          onClick={() => {
                            ...
                            if (current !== BookingMode.REQUEST_TO_BOOK) {
                              updateBookingMode(
                                {
                                  propertyId,
                                  booking_mode: BookingMode.REQUEST_TO_BOOK,
                                },
                                {
                                  onSuccess: () => {
                                    setProperty((prev) => ({
                          ...
                          icon={
                            <Icon
                              icon="solar:hand-shake-bold-duotone"
                              className={`text-base ${(property?.bookingMode ?? property?.booking_mode) === BookingMode.REQUEST_TO_BOOK ? "text-violet-600" : "text-zinc-400"}`}
                            />
                          }
                          iconBg={
                            (property?.bookingMode ??
            ...
            >
              <div className="w-full p-1 flex flex-col gap-6">
                ...
                <ModalSection title="Agent feedback">
                  <p className="text-sm text-zinc-700 mt-1.5">
                    {property?.verifications?.[0]?.feedback ?? (
                      <em className="text-zinc-400 font-normal">
                        No comments yet
                      </em>
                    )}
                  </p>
                ...
                <ModalSection title="KYC details">
                  <p className="text-sm text-zinc-400 italic mt-1.5">
                    Coming soon...
                  </p>
                </ModalSection>
              </div>
            ...
            >
              <div className="w-full p-1">
                {!selectedAgent ? (
                  <div className="mt-2">
                    <label className="text-sm text-zinc-600 font-medium block mb-2">
                      Search agents
                    </label>
                  ...
                  <div className="mt-2 space-y-4">
                    ...
                    <div className="flex gap-3 pt-1">
                      ...
                      <button
                        onClick={() =>
                          handleAgentAssignment(String(selectedAgent?.id))
                        }
                        disabled={assignmentLoading}
                        type="button"
                  ...
                  <div className="mt-2 space-y-4">
                    <div className="flex gap-3 items-center p-3 bg-zinc-50 rounded-xl">
                      <Image
                        alt="owner-image"
                        src={
                          (selectedOwner?.profile?.profileImage ||
                            selectedOwner?.profile?.profile_image) ??
                          "/png/sample_profile.png"
                        }
                        height={44}
                        width={44}
                    ...
                    <p className="text-sm text-zinc-700">
                      Reassign this property to{" "}
                      <strong>
                        {selectedOwner?.firstName
                          ? `${selectedOwner?.firstName} ${selectedOwner?.lastName}`
                          : selectedOwner?.email || "this owner"}
                      </strong>
                      ?
                    </p>
                    <div className="flex gap-3 pt-1">
                      ...
                      <button
                        onClick={() =>
                          handleOwnerReassignment(String(selectedOwner?.id))
                        }
                        disabled={reassignOwnerLoading}
                        type="button"
                        className="flex-1 py-2.5 rounded-xl text-sm font-semibold bg-primary text-white hover:bg-primary/90 disabled:opacity-60 transition-all"
            ...
            >
              <div className="w-full space-y-4 sm:space-y-5 pt-2 sm:pt-3">
                <div className="bg-zinc-50 p-2 sm:p-3 rounded-lg sm:rounded-xl">
                  <p className="text-[10px] sm:text-xs font-bold text-zinc-400 uppercase mb-1">
                    Document Type
                  </p>
                  <p className="text-xs sm:text-sm font-bold text-zinc-800">
                    {(selectedDoc?.document_type as string)
                      ?.replace(/_/g, " ")
                      .toLowerCase()
                      .replace(/\b\w/g, (c: string) => c.toUpperCase())}
                  </p>
  ...
  return (
    <div className="flex items-center justify-between py-1.5">
      <div className="flex items-center gap-2.5">
        <div className="p-1.5 bg-zinc-50 rounded-lg flex-shrink-0">{icon}</div>
    ...
    <div className="flex items-center gap-3">
      <div className="relative flex-shrink-0">
        <Image
          ...
          height={40}
          width={40}
          className="rounded-full object-cover ring-2 ring-zinc-100 w-10 h-10"
        />
        <div
          className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 ${dotColor} border-2 border-white rounded-full`}
        />
      </div>
      <div className="flex-1 min-w-0">
    ...
    >
      ...
      {showCheck && (
        <Icon
          ...
          className={`${checkColor} text-lg shrink-0 mt-0.5`}
        />
      )}
    </button>
  );
