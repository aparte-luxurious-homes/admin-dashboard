...
export default function UnitDetailsView({
  ...
  const [propertyUnit, setPropertyUnit] = useState<IPropertyUnit>(
    data?.data?.data,
  );
  ...
  const { data: availabilityData } = GetUnitAvailability(
    propertyId,
    unitId,
    startDate,
    endDate,
  );
  const { mutate: saveAvailability, isPending: isSavingAvailability } =
    CreateUnitAvailability();
  ...
  const handleDelete = () => {
    dispatch(
      showAlert({
        title: "Are you sure?",
        description:
          "This action cannot be undone. This will permanently delete this property unit.",
        confirmText: "Delete",
        cancelText: "Cancel",
        onConfirm: () => {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 lg:gap-8">
            ...
            <div className="lg:col-span-8 space-y-6 sm:space-y-8 lg:space-y-10">
                ...
                >
                  {propertyUnit?.media?.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-[250px] sm:h-[350px] md:h-[450px] text-zinc-300">
                      ...
                      <p className="font-semibold italic text-base sm:text-lg md:text-xl px-4 text-center">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start gap-3 sm:gap-4">
                  ...
                  <div className="bg-primary px-2 sm:px-3 md:px-3 py-2 sm:py-2 rounded-1xl sm:rounded-2xl shadow-md sm:shadow-xl shadow-primary/20 text-center w-full sm:w-auto sm:min-w-[160px] md:min-w-[180px]">
                    <p className="text-xl sm:text-1xl md:text-[20px] font-bold text-white">
                      {formatMoney(
                        propertyUnit?.pricePerNight ||
                          propertyUnit?.price_per_night ||
                          0,
                      )}
                    </p>
                    <p className="text-[8px] sm:text-[9px] md:text-[10px] font-bold text-white/70 uppercase tracking-[0.2em] mt-0.5 sm:mt-1">
                      Per Night
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-zinc-50 rounded-2xl sm:rounded-3xl p-4 sm:p-3 md:p-4 border border-zinc-100 relative overflow-hidden">
                  ...
                  <p className="text-[12px] sm:[14px] md:[14px] text-zinc-600 leading-relaxed max-w-3xl relative z-10">
                    {propertyUnit?.description ||
                      "No description has been added for this unit yet."}
                  </p>
                </div>
              </section>
                  ...
                  {[
                    ...
                    },
                    {
                      ...
                      value:
                        ...
                        propertyUnit?.living_room_count ||
                        0,
                      icon: LuSofa,
                    },
                  ].map((feature, i) => (
                    <div
                      key={i}
                      className="p-1 sm:p-1 md:p-1 lg:p-1 bg-white border border-zinc-200 rounded-[4px] sm:rounded-[8px] md:rounded-[1rem] flex flex-col items-center gap-2 sm:gap-3 md:gap-4 hover:border-primary/40 hover:shadow-lg transition-all group cursor-default"
                    >
                      ...
                      <div className="text-center">
                        <p className="text-[8px] sm:text-[9px] md:text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-0.2 sm:mb-0.5">
                          {feature.label}
                        </p>
                        <p className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-zinc-900 leading-none">
                          {feature.value}
                        </p>
                      </div>
                    </div>
              ...
              {propertyUnit?.amenities &&
                propertyUnit?.amenities?.length > 0 && (
                  <section className="space-y-4 sm:space-y-5 md:space-y-6">
                    <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-zinc-900 flex items-center gap-2 sm:gap-3">
                      <Icon
                    ...
                    <div className="flex flex-wrap gap-2 sm:gap-3">
                      {propertyUnit?.amenities?.map((el, index) => (
                        <div
                          key={index}
                          className="px-3 sm:px-4 md:px-5 lg:px-6 py-2 sm:py-3 md:py-4 bg-white border border-zinc-100 rounded-lg sm:rounded-xl md:rounded-2xl shadow-sm text-xs sm:text-sm font-bold text-zinc-700 flex items-center gap-1 sm:gap-2 hover:border-primary/20 transition-colors"
                        >
                  ...
                  {propertyUnit?.reviews?.length === 0 ? (
                    <div className="p-8 sm:p-12 md:p-16 bg-zinc-50/50 border border-dashed border-zinc-200 rounded-2xl sm:rounded-3xl md:rounded-[3rem] flex flex-col items-center justify-center text-zinc-400">
                      <Icon
                        ...
                        className="text-4xl sm:text-5xl md:text-6xl mb-2 sm:mb-3 md:mb-4 opacity-20"
                      />
                      <p className="font-bold text-sm sm:text-base md:text-lg opacity-60 text-center">
                    ...
                    propertyUnit?.reviews?.map((review, index) => (
                      <div
                        key={index}
                        className="p-4 sm:p-6 md:p-8 bg-white border border-zinc-100 rounded-xl sm:rounded-2xl md:rounded-3xl shadow-sm hover:shadow-md transition-shadow"
                      >
                        <div className="flex items-center gap-3 sm:gap-4 md:gap-5 mb-3 sm:mb-4 md:mb-6">
                          <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-zinc-100 rounded-full flex items-center justify-center ring-2 sm:ring-4 ring-zinc-50">
                            <LuUsers
                              className="text-zinc-400 sm:w-5 md:w-6"
                              size={16}
                            />
                          </div>
                          <div>
                            <p className="font-bold text-sm sm:text-base md:text-lg text-zinc-900 leading-tight">
                              Verified Guest
                            </p>
                            <p className="text-[8px] sm:text-[10px] md:text-xs font-bold text-zinc-400 uppercase tracking-widest mt-0.5">
                              Confirmed Booking
                            </p>
                          </div>
                        </div>
                        <p className="text-sm sm:text-base md:text-lg text-zinc-600 leading-relaxed italic line-clamp-3">
                          "{review?.review}"
                        </p>
                      </div>
                    ))
            ...
            <div className="lg:col-span-4 space-y-4 sm:space-y-6 lg:space-y-8 mt-4 sm:mt-6 lg:mt-0">
              ...
              <div className="bg-white border border-zinc-200 rounded-2xl sm:rounded-3xl md:rounded-[2rem] lg:rounded-[1rem] p-2 sm:p-1 md:p-2 lg:p-4 shadow-md sm:shadow-lg lg:shadow-xl shadow-zinc-200/50 space-y-5 sm:space-y-6 lg:space-y-8">
                    ...
                    {[
                      {
                        icon: LuUsers,
                        label: "Capacity",
                        value: `${propertyUnit?.max_guests || 0} Guests`,
                      },
                      ...(propertyUnit?.seating_capacity
                        ? [
                            {
                              icon: "solar:chair-bold-duotone",
                              label: "Seating Capacity",
                              value: `${propertyUnit.seating_capacity}`,
                            },
                          ]
                        : []),
                      ...(propertyUnit?.standing_capacity
                        ? [
                            {
                              icon: "solar:users-group-rounded-bold-duotone",
                              label: "Standing Capacity",
                              value: `${propertyUnit.standing_capacity}`,
                            },
                          ]
                        : []),
                      ...(propertyUnit?.car_park_spaces
                        ? [
                            {
                              icon: "solar:car-bold-duotone",
                              label: "Car Park Spaces",
                              value: `${propertyUnit.car_park_spaces}`,
                            },
                          ]
                        : []),
                      ...(propertyUnit?.power_supply_provision
                        ? [
                            {
                              icon: "solar:bolt-bold-duotone",
                              label: "Power Supply",
                              value: `${propertyUnit.power_supply_provision}`,
                            },
                          ]
                        : []),
                      {
                        ...
                        label: "Caution Fee",
                        value: `${formatMoney(propertyUnit?.caution_fee || 0)}`,
                      },
                      {
                        icon: "solar:box-bold-duotone",
                        label: "Available Inventory",
                        value: `${propertyUnit?.count || 1} Units`,
                      },
                    ].map((spec, i) => (
                      <div
                        key={i}
                        className="flex justify-between items-center group/spec"
                      >
                        <div className="flex items-center gap-2 sm:gap-3 lg:gap-4">
                          <div className="p-1.5 sm:p-2 lg:p-2.5 bg-zinc-50 rounded-lg sm:rounded-xl group-hover/spec:bg-primary/10 transition-colors">
                            {typeof spec.icon === "string" ? (
                              <Icon
                                icon={spec.icon}
                                className="text-zinc-500 group-hover/spec:text-primary transition-colors text-base sm:text-lg lg:text-xl"
                              />
                            ) : (
                              <spec.icon className="text-zinc-500 group-hover/spec:text-primary transition-colors text-base sm:text-lg lg:text-xl" />
                            )}
                          </div>
                          <span className="text-zinc-600 font-bold text-xs sm:text-sm tracking-tight">
                            {spec.label}
                          </span>
                        </div>
                ...
                <div className="space-y-2 pt-2 sm:pt-3 lg:pt-4">
                  <Link
                    href={PAGE_ROUTES.dashboard.propertyManagement.allProperties.units.bookings(
                      propertyUnit?.propertyId ||
                        propertyUnit?.property_id ||
                        propertyId,
                      propertyUnit?.id,
                      ...
                      onSave={(dates) => {
                        saveAvailability(
                          ...
                          },
                          {
                            onSuccess: () =>
                              toast.success(
                                MESSAGES.MSG_AVAILABILITY_UPDATED_SUCCESSFULLY,
                              ),
                            onError: () =>
                              toast.error(
                                MESSAGES.MSG_FAILED_TO_UPDATE_AVAILABILITY,
                              ),
                          },
                        );
                      }}
                  ...
                  <div className="space-y-4 sm:space-y-6 lg:space-y-10">
                    {[
                      { label: "OWNER", user: propertyUnit?.property?.owner },
                      {
                        ...
                        user: propertyUnit?.property?.agent,
                      },
                    ].map((role, i) => (
                      <div key={i} className="group">
                        <p className="text-[8px] sm:text-[9px] lg:text-[10px] font-bold text-zinc-400 uppercase tracking-[0.2em] mb-2 sm:mb-3 lg:mb-4">
                          {role.label}
                        </p>
                        <div className="flex gap-2 sm:gap-3 lg:gap-4 items-center">
                          <div className="relative flex-shrink-0">
                            <Image
                              ...
                              src={
                                (role.user?.profile?.profileImage ||
                                  role.user?.profile?.profile_image) ??
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-bold text-zinc-900 truncate text-sm sm:text-base">
                              {role.user?.profile?.firstName
                                ? `${role.user?.profile?.firstName} ${role.user?.profile?.lastName}`
                                : role.user?.email || "System User"}
                            </p>
                            <p className="text-[10px] sm:text-xs font-bold text-zinc-400 truncate">
                              {role.user?.email}
                            </p>
                          </div>
                        </div>
