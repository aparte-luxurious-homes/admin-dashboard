...
export default function CalendarSyncView({
  ...
  // Outbound state
  const { data: outboundData, isLoading: isLoadingOutbound } =
    GetOutboundUrl(unitId);
  const { mutate: rotateOutbound, isPending: isRotating } = RotateOutboundUrl();
  const outboundUrl =
    outboundData?.data?.data?.feed_url || outboundData?.data?.data?.url || "";

  // Inbound state
  const { data: inboundData, isLoading: isLoadingInbound } =
    GetInboundFeeds(unitId);
  ...
  const feeds = Array.isArray(rawFeeds)
    ? rawFeeds.filter((feed: any) => feed.label)
    : [];
  ...
  const handleRotate = () => {
    dispatch(
      showAlert({
        title: "Are you sure?",
        description:
          "This will instantly break the sync with any external platforms (like Airbnb) where you previously pasted this link. You will need to copy the new link and update it everywhere.",
        confirmText: "Regenerate",
        cancelText: "Cancel",
        onConfirm: () => {
          rotateOutbound(unitId, {
            onSuccess: () =>
              toast.success(MESSAGES.MSG_OUTBOUND_LINK_REGENERATED),
          });
        },
      }),
    );
  ...
  const handleAddFeed = (e: React.FormEvent) => {
    ...
    addFeed(
      { unitId, payload: { url: newFeedUrl, label: newFeedLabel } },
      {
        onSuccess: () => {
        ...
        onError: (err: any) => {
          ...
          toast.error(
            typeof finalMsg === "string" ? finalMsg : "Failed to add feed.",
          );
        },
      },
    );
  ...
  const handleDelete = (feedId: string | number) => {
    dispatch(
      showAlert({
        title: "Remove feed?",
        description:
          "This will remove the feed and all its imported blocked dates. This action cannot be undone.",
        confirmText: "Remove",
        cancelText: "Cancel",
        onConfirm: () => {
          deleteFeed(
    ...
    <div className="space-y-8 mt-10">
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl p-6 shadow-sm space-y-4">
        ...
        <p className="text-sm text-zinc-600 max-w-3xl">
          Copy this link and paste it into external platforms like Airbnb or
          Booking.com so they can block dates when someone books on Aparte.
        </p>
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl p-6 shadow-sm space-y-4">
        ...
        <p className="text-sm text-zinc-600 max-w-3xl">
          Paste links from external platforms here so Aparte can block those
          dates on your calendar to prevent double-bookings.
        </p>
                ...
                {feeds.map((feed: any) => (
                  <tr key={feed.id} className="hover:bg-zinc-50">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {!feed.is_active && (
                        <span className="ml-2 px-2 py-1 bg-zinc-100 text-zinc-600 rounded-full text-[10px] font-bold">
                          DISABLED
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-zinc-500">
                      {feed.last_polled_at
                        ? format(new Date(feed.last_polled_at), "PP p")
                        : "Never"}
                    </td>
                    <td className="px-4 py-3">
