...
function NumberInput({
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (rawValue === '') { setDisplayValue(''); return; }
        ...
        <div className="relative">
            ...
            <input
                ...
                className={`w-full bg-zinc-50 border rounded-lg pl-9 pr-16 py-2.5 outline-none transition-all font-semibold text-zinc-900 text-sm ${
                    isFocused ? 'border-primary ring-1 ring-primary/20' : 'border-zinc-200 hover:border-zinc-300'
                }`}
            />
            <div className="absolute inset-y-0 right-0 flex items-center gap-0.5 pr-1">
                <button type="button" onClick={decrement} className="p-1 rounded bg-zinc-100 hover:bg-zinc-200 text-zinc-600 transition-colors">
                    <FaMinus className="text-[8px]" />
                </button>
                <button type="button" onClick={increment} className="p-1 rounded bg-primary/10 hover:bg-primary/20 text-primary transition-colors">
...
export default function UnitDrawer({
    ...
    useEffect(() => {
        if (isOpen) {
            setUnit(editingUnit ?? createEmptyUnit());
        }
    }, [isOpen, editingUnit]);
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex items-center justify-center gap-1 bg-zinc-900 rounded-xl py-3">
                            ...
                            <span className="text-lg font-bold text-white tracking-tight">
                                {formatMoney(Number(unit.price_per_night || 0) + Number(unit.caution_fee || 0))}
                            </span>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-zinc-100">
                            <CustomCheckbox
                                ...
                                onChange={(val: boolean) => updateField('is_whole_property', val)}
                            />
                            <div className="w-24">
                                <label className="text-[8px] font-medium text-zinc-500 uppercase tracking-wider ml-1">
                                    Units
