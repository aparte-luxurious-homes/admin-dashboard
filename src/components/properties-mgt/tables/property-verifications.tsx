...
import { GetPropertyVerifications, UpdatePropertyVerification } from "@/src/lib/request-handlers/propertyMgt";
...
export default function PropertyVerificationsTable({
    ...
    const { data: verificationList, isLoading: verificationsLoading, refetch } = GetPropertyVerifications(page, 10, searchTerm, propertyId)
                ...
                onConfirm: () => {
                    updateVerification(
                        ...
                        },
                        {
                            ...
                            onError: (error: any) => {
                                toast.error(error?.response?.data?.detail || error?.response?.data?.message || 'Failed to verify property', {
                                    duration: 6000,
                                    style: { maxWidth: '500px', width: 'max-content' }
                                });
                            }
    ...
    const detailButtons = [
        {
            ...
            onClick: () => {
                router.push(
                    PAGE_ROUTES.dashboard.propertyManagement.allProperties.verifications.details(propertyId, verifications[selectedRow!]?.id)
                )
                setSelectedRow(null)
            },
        },
        {
            ...
            onClick: () => {
                router.push(
                    `${PAGE_ROUTES.dashboard.propertyManagement.allProperties.verifications.details(propertyId, verifications[selectedRow!]?.id)}?edit=true`
                )
                setSelectedRow(null)
            },
        },
                ...
                <div className="w-full border border-zinc-500/20 bg-white rounded-xl sm:rounded-2xl px-3 sm:px-4 md:px-6 py-4 sm:py-5 md:py-6 lg:py-7 min-h-[70vh] flex flex-col items-center">
                    ...
                    <div className="w-full flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 mb-4 sm:mb-5">
                        <div className="w-full sm:w-auto flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-5">
                            <p className="text-lg sm:text-xl md:text-2xl font-medium">Property verifications</p>
                    ...
                    {verificationsLoading ? (
                                ...
                                <table className="w-full border-collapse">
                                    ...
                                    <tbody className="text-[11px] sm:text-xs md:text-sm">
                                        {verifications?.map((verification, index) => {
                                            const vPropertyId = verification?.propertyId ?? verification?.property_id ?? verification?.property?.id;
                                            const formattedId = vPropertyId ? `APRT25-${vPropertyId}` : '--/--';
                                            ...
                                            return (
                                                <tr
                                                    ...
                                                    onClick={() => {
                                                        router.push(PAGE_ROUTES.dashboard.propertyManagement.allProperties.verifications.details(vPropertyId ?? propertyId, verification?.id));
                                                    }}
                                                >
                                                    <td className="px-2 sm:px-3 md:px-4 py-3 sm:py-4 border-b border-b-gray-200">
                                                        <span className="text-xs sm:text-sm font-medium truncate max-w-[120px] block">
                                                            {formattedId}
                                                        </span>
                                                    </td>
                                                    <td className="px-2 sm:px-3 md:px-4 py-3 sm:py-4 border-b border-b-gray-200">
                                                        <p className="text-xs sm:text-sm truncate max-w-[150px] md:max-w-[180px]">
                                                            {verification?.property?.name || '--/--'}
                                                        </p>
                                                    </td>
                                                    <td className="px-2 sm:px-3 md:px-4 py-3 sm:py-4 border-b border-b-gray-200">
                                                        <p className="text-xs sm:text-sm truncate max-w-[150px] md:max-w-[180px]">
                                                            {verification?.feedback || <em className="text-zinc-400">No comments</em>}
                                                        </p>
                                                    </td>
                                                    <td className="px-2 sm:px-3 md:px-4 py-3 sm:py-4 border-b border-b-gray-200">
                        ...
                        <div className="flex flex-col items-center justify-center py-10 sm:py-12 md:py-16 w-full">
                            <div className="w-fit mb-3">
                                <Icon icon="hugeicons:album-not-found-01" width="32" height="32" className="sm:w-10 sm:h-10 text-gray-400" />
                            </div>
                            <p className="text-center text-gray-500 text-xs sm:text-sm">No verifications found</p>
                        </div>
                    ) : (
                        <div className="flex flex-col items-center justify-center py-10 sm:py-12 md:py-16 w-full">
                            <div className="w-fit mb-3">
                                <Icon icon="mynaui:danger-octagon" width="32" height="32" className="sm:w-10 sm:h-10 text-red-600" />
                            </div>
                            <p className="text-center text-gray-500 text-xs sm:text-sm">Error loading verifications</p>
                        </div>
                    )}
                </div>
                ...
                {selectedRow !== null && modalPosition && (
                    ...
                    >
                        {detailButtons.map((button, idx) => (
                            ...
                            >
                                <span className={`${button.label === 'Reject' ? 'text-red-500' : button.label === 'Verify' ? 'text-green-500' : 'text-gray-500'}`}>
                                    {button.Icon}
                                </span>
                        ...
                        <>
                            <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-100">
                                <p className="text-sm text-blue-800">
                                    <span className="font-semibold">Property:</span> {selectedVerification?.property?.name || 'N/A'}
                                </p>
                            </div>

                            <div className="mb-4">
                                <label className="block text-sm font-medium text-zinc-700 mb-2">
                                    Feedback / Notes <span className="text-zinc-400 text-xs">(optional)</span>
                                </label>
                                <textarea
                                    rows={4}
                                    value={feedbackText}
                            ...
                            <div className="flex justify-end gap-3 mt-6">
                                ...
                                >
                                    {verificationUpdating ? <div className="w-4 h-4"><Loader /></div> : <MdOutlineVerified />}
                    ...
                    {selectedVerification && (
                        <>
                            ...
                            <div className="flex justify-end gap-3 mt-6">
                                ...
                                >
                                    {verificationUpdating ? <div className="w-4 h-4"><Loader /></div> : <ImCancelCircle />}
