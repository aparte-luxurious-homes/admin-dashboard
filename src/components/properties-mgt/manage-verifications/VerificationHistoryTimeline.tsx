...
function triggerIcon(trigger: string): string {
    switch (trigger) {
        ...
        case "AGENT_DECISION":  return "mdi:home-search";
        case "OWNER_RESUBMIT":  return "mdi:refresh";
        case "DOC_UPLOAD":      return "mdi:file-upload-outline";
        case "REWARD_REVERSED": return "mdi:cash-refund";
...
function triggerLabel(trigger: string): string {
    switch (trigger) {
        ...
        case "AGENT_DECISION":  return "Agent on-site decision";
        case "OWNER_RESUBMIT":  return "Owner resubmission";
        case "DOC_UPLOAD":      return "Document review";
        case "REWARD_REVERSED": return "Reward reversed";
...
const VerificationHistoryTimeline: React.FC<Props> = ({ propertyId, verificationId }) => {
    const { data, isLoading } = GetVerificationHistory(propertyId, verificationId);
    ...
    return (
        <section className="my-6">
            <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-lg font-bold text-gray-800">Verification Activity</h4>
            </div>
                                ...
                                <li key={it.id} className="ml-6">
                                    ...
                                    <div className="space-y-1.5">
                                        <div className="flex items-center gap-2 flex-wrap">
                                            ...
                                            {it.evidence_count !== null && it.evidence_count !== undefined && it.trigger === "AGENT_DECISION" && (
                                                <span className="text-xs text-gray-500">· {it.evidence_count} evidence</span>
                                            )}
                                        </div>
                                        <p className="text-sm text-gray-700">
                                            <span className="font-medium">{it.performer_label || "System"}</span>
                                            {it.performer_role && <span className="text-gray-500"> ({it.performer_role})</span>}
                                        ...
                                        {it.feedback && (
                                            <p className="text-xs text-red-700 bg-red-50 border border-red-100 rounded px-2 py-1.5">
                                                <span className="font-semibold">Feedback:</span> {it.feedback}
                                            </p>
                                        )}
                                        {skipParts.length > 0 && (
                                        ...
                                        {it.trigger === "REWARD_REVERSED" && it.reward_amount !== null && (
                                            <p className="text-xs text-orange-700 bg-orange-50 border border-orange-100 rounded px-2 py-1.5">
                                                Debited NGN {Number(it.reward_amount).toLocaleString()} from the agent who originally got credited.
                                            </p>
                                        )}
                                        {it.trigger !== "REWARD_REVERSED" && it.reward_amount !== null && it.reward_amount !== undefined && (
                                            <p className="text-xs text-green-700 bg-green-50 border border-green-100 rounded px-2 py-1.5">
                                                Credited NGN {Number(it.reward_amount).toLocaleString()} as verification reward.
                                            </p>
                                        )}
                                    </div>
                                </li>
