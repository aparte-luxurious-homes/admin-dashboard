...
export default function SideNav({ link }: { link: ILink }) {
        ...
        <div className="relative w-full cursor-pointer">
            {
                <div 
                    key={link.key}
                    className="mb-3 rlinkative"
                    onClick={() => setOpen(!open)}
                >
                    <div className="w-full">
                        <div 
                            onClick={() => !link.secondary && router.push(link.link)}
                            className="flex items-center gap-4 pt-3 pb-2 pl-12 hover:bg-teal-600/60">
                            {link.icon}
                        ...
                        <div className={`flex flex-col w-full transition-all ease-in-out duration-150 ${open ? 'max-h-40 opacity-100': 'max-h-0 opacity-0'}`}>
                            {
                                ...
                                link.children?.map((child) => 
                                    <Link key={child.key} href={child.link} className="flex items-center gap-4 pl-[5.2rem] py-2 hover:bg-teal-600/60">
