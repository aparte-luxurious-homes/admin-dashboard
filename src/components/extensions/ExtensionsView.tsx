...
const ExtensionsView = () => {
    ...
    const handleAction = async (action: 'approve' | 'reject' | 'cancel', ext: any) => {
        ...
        if (action === 'approve') {
            if (confirm("Are you sure you want to approve this extension stay request?")) {
            ...
            if (confirm("Are you sure you want to cancel this extension stay request?")) {
    ...
    const getStatusStyle = (status: ExtensionStatus) => {
        switch (status) {
            case ExtensionStatus.PENDING_PAYMENT: return "bg-amber-100 text-amber-700 ring-1 ring-amber-600/20";
            case ExtensionStatus.AWAITING_OWNER_APPROVAL: return "bg-purple-100 text-purple-700 ring-1 ring-purple-600/20";
            case ExtensionStatus.APPROVED: return "bg-blue-100 text-blue-700 ring-1 ring-blue-600/20";
            case ExtensionStatus.CONFIRMED: return "bg-green-100 text-green-700 ring-1 ring-green-600/20";
            case ExtensionStatus.REJECTED: return "bg-red-100 text-red-700 ring-1 ring-red-600/20";
            case ExtensionStatus.CANCELLED: return "bg-gray-100 text-gray-700 ring-1 ring-gray-600/20";
            ...
            <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-gray-200 bg-gray-50/50">
                    <div className="flex justify-between items-center gap-4 flex-wrap mb-6">
                        <div>
                            <h1 className="text-xl font-semibold text-gray-900">Stay Extensions</h1>
                            <p className="text-sm text-gray-500 mt-1">Manage guest requests to extend their stay period</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="ml-auto bg-white px-4 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-600 shadow-sm">
                            Total Requests: <span className="text-primary">{totalCount}</span>
                        </div>
                    </div>
                </div>
                    ...
                    {isLoading ? (
                            ...
                            <tbody className="divide-y divide-gray-200 bg-white">
                                {extensions.map((ext: any, index: number) => (
                                    <tr 
                                        ...
                                        onClick={() => router.push(PAGE_ROUTES.dashboard.bookingManagement.bookings.details(ext.booking_id))}
                                    >
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase ${getStatusStyle(ext.status)}`}>
                        ...
                        <div className="flex flex-col items-center justify-center py-20 bg-white">
                            <div className="w-16 h-16 rounded-full bg-gray-50 flex items-center justify-center mb-4">
                                <Icon icon="solar:calendar-search-bold" width="32" height="32" className="text-gray-300" />
                            </div>
                            <h3 className="text-lg font-medium text-gray-900 mb-1">No extension requests found</h3>
                            <p className="text-sm text-gray-500">Wait for guests to request more time or check back later</p>
                        </div>
                    )}
                </div>
                ...
                >
                    ...
                    <button
                        ...
                        onClick={(e) => {
                            ...
                            router.push(PAGE_ROUTES.dashboard.bookingManagement.bookings.details(ext.booking_id));
                            setSelectedRow(null);
                        }}
                    >
                        <Icon icon="solar:eye-bold" className="text-zinc-400 group-hover:text-primary transition-colors" width="18" />
                        <span>View Details</span>
                    </button>

                    {(isAdmin || isOwner) && extensions[selectedRow].status === ExtensionStatus.AWAITING_OWNER_APPROVAL && (
                        <>
                            <button
                            ...
                            >
                                <Icon icon="solar:check-circle-bold" className="text-green-400 group-hover:text-green-600 transition-colors" width="18" />
                            ...
                            >
                                <Icon icon="solar:close-circle-bold" className="text-red-400 group-hover:text-red-600 transition-colors" width="18" />
                                <span>Reject Request</span>
                            </button>
                        </>
                    )}

                    {isAdmin && [ExtensionStatus.APPROVED, ExtensionStatus.PENDING_PAYMENT, ExtensionStatus.AWAITING_OWNER_APPROVAL].includes(extensions[selectedRow].status) && (
                        <button
                        ...
                        >
                            <Icon icon="solar:trash-bin-trash-bold" className="text-amber-400 group-hover:text-amber-600 transition-colors" width="18" />
