...
import {
  ...
  UpdateUser,
  GetAssignableRoles,
} from "@/src/lib/request-handlers/userMgt";
...
const UserManagementView = ({
  ...
  useEffect(() => {
    if (
      ...
      !assignableRoles.includes(createForm.role)
    ) {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || `Failed to fetch ${title.toLowerCase()}`,
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <div className="p-4 sm:p-5 md:p-6 border-b border-gray-200 bg-gray-50/50">
                  ...
                  {isOpen && (
                    <div className="absolute right-0 mt-2 w-40 sm:w-48 bg-white border border-gray-200 rounded-lg shadow-xl z-20 overflow-hidden">
                      <button
                        ...
                        className="w-full text-left px-3 sm:px-4 py-2.5 text-xs sm:text-sm hover:bg-gray-50 transition-colors border-b border-gray-100"
                      >
                        Export as CSV
                      </button>
                      <button
            ...
            <div className="sm:hidden bg-white px-3 py-2 rounded-lg border border-gray-200 text-xs font-medium text-gray-600">
              Total {role.charAt(0) + role.slice(1).toLowerCase()}s:{" "}
              <span className="text-primary">{rowCount}</span>
            </div>
          </div>
                    ...
                    >
                      ...
                      <td className="px-3 sm:px-6 py-3 sm:py-4 text-gray-700">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[8px] sm:text-xs text-gray-500 truncate max-w-[80px] sm:max-w-none">
                            {user.phone || "--"}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-3 sm:px-6 py-3 sm:py-4 text-center">
                        <Badge
                          status={user.is_verified ?? user.isVerified ?? false}
                        />
                      </td>
      ...
      {isCreateOpen && (
        <div
          className="fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-3 sm:p-4 backdrop-blur-sm transition-all duration-300"
          onClick={() => setIsCreateOpen(false)}
        >
          <div
            className="bg-white rounded-xl sm:rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200"
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 sm:gap-4">
                  <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-zinc-100 rounded-xl border border-zinc-200">
                    ...
                    >
                      <span
                        className={`${isQuickOnboard ? "translate-x-5" : "translate-x-1"} inline-block h-3 w-3 transform rounded-full bg-white transition-transform`}
                      />
                    </button>
                  </div>
                  <button
                    onClick={() => setIsCreateOpen(false)}
                    className="p-1.5 sm:p-2 hover:bg-white hover:shadow-sm rounded-xl transition-all duration-200 group"
                  >
                    <Icon
                      icon="mdi:close"
                      width="18"
                      height="18"
                      className="sm:w-5 sm:h-5 text-gray-400 group-hover:text-gray-600"
                    />
                  </button>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 custom-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
                ...
                <div className="md:col-span-2 space-y-1.5 sm:space-y-2">
                  ...
                  <p className="text-[8px] sm:text-[10px] text-gray-400 ml-1 italic">
                    Format: +234XXXXXXXXXX or 080XXXXXXXX
                  </p>
                </div>
                ...
                {!isQuickOnboard && (
                  <div className="md:col-span-2 space-y-1.5 sm:space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                    ...
                    <div className="p-3 sm:p-4 bg-gray-50/50 rounded-lg sm:rounded-xl border border-gray-100 mt-2">
                      <p className="text-[9px] sm:text-[11px] font-bold text-gray-500 mb-2 uppercase tracking-tight">
                        Security Strength:
                      </p>
                      <div className="grid grid-cols-2 gap-2 sm:gap-4">
                        <p
                          className={`text-[8px] sm:text-[10px] flex items-center gap-1 sm:gap-1.5 font-medium ${createForm.password.length >= 8 ? "text-green-600" : "text-gray-400"}`}
                        >
                          <Icon
                            icon={
                        ...
                        <p
                          className={`text-[8px] sm:text-[10px] flex items-center gap-1 sm:gap-1.5 font-medium ${/[A-Z]/.test(createForm.password) ? "text-green-600" : "text-gray-400"}`}
                        >
                          <Icon
                            icon={
                        ...
                        <p
                          className={`text-[8px] sm:text-[10px] flex items-center gap-1 sm:gap-1.5 font-medium ${/\d/.test(createForm.password) ? "text-green-600" : "text-gray-400"}`}
                        >
                          <Icon
                            icon={
                        ...
                        <p
                          className={`text-[8px] sm:text-[10px] flex items-center gap-1 sm:gap-1.5 font-medium col-span-2 ${/[!@#$%^&*()_+\-=[\]{}|;:,.<>?]/.test(createForm.password) ? "text-green-600" : "text-gray-400"}`}
                        >
                          <Icon
                            icon={
                              /[!@#$%^&*()_+\-=[\]{}|;:,.<>?]/.test(
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    <div className="p-3 sm:p-4 bg-gray-50 rounded-xl sm:rounded-2xl border border-gray-100 flex items-center justify-between group hover:border-primary/20 transition-all duration-200">
                      <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[8px] sm:text-xs text-gray-500 font-medium">
                            Allow user to login
                          </p>
                        </div>
                      </div>
                      ...
                      >
                        <span
                          className={`${createForm.is_active ? "translate-x-5 sm:translate-x-6" : "translate-x-1"} inline-block h-3 sm:h-4 w-3 sm:w-4 transform rounded-full bg-white transition-transform`}
                        />
                      </button>
                    </div>
                    ...
                    <div className="p-3 sm:p-4 bg-gray-50 rounded-xl sm:rounded-2xl border border-gray-100 flex items-center justify-between group hover:border-primary/20 transition-all duration-200">
                      <div className="flex items-center gap-2 sm:gap-3">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-white flex items-center justify-center shadow-sm">
                          <Icon
                            icon="mdi:verified-badge-outline"
                            width="16"
                            className="sm:w-5 sm:h-5"
                          />
                        </div>
                      ...
                      >
                        <span
                          className={`${createForm.isVerified ? "translate-x-5 sm:translate-x-6" : "translate-x-1"} inline-block h-3 sm:h-4 w-3 sm:w-4 transform rounded-full bg-white transition-transform`}
                        />
                      </button>
                    </div>
                  </div>
                  ...
                  action(
                    {
                      payload: {
                        ...
                        gender: createForm.gender
                          ? createForm.gender.toUpperCase()
                          : undefined,
                        password: isQuickOnboard
                          ? undefined
                          : createForm.password,
                      },
                    },
