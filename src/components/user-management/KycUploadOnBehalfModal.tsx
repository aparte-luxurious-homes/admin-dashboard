...
const KycUploadOnBehalfModal = ({
  ...
  const [documentType, setDocumentType] = useState<string>(
    KycDocumentType.INTERNATIONAL_PASSPORT,
  );
  ...
  const fileError = (() => {
    ...
    if (!ALLOWED_MIME.includes(file.type))
      return `Unsupported type "${file.type}". Allowed: JPG, PNG, WEBP, PDF.`;
    if (file.size > MAX_BYTES)
      return `File is ${(file.size / (1024 * 1024)).toFixed(1)}MB. Max 10MB.`;
    return "";
  })();
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div className="flex items-start gap-3 p-3 bg-blue-50 border border-blue-100 rounded-lg">
          ...
          <div className="text-sm text-blue-900">
            ...
            attached to their KYC record as{" "}
            <span className="font-semibold">PENDING</span> — review and verify
            on the same panel after upload.
          </div>
        </div>
        ...
        <div>
          <label className="text-sm font-medium text-gray-700 block mb-1">
            Internal notes{" "}
            <span className="text-gray-400">(optional, audit-only)</span>
          </label>
          <textarea
            value={notes}
