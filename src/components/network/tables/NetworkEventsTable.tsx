...
export default function NetworkEventsTable() {
  ...
  useEffect(() => {
    ...
    axiosRequest
      .get(API_ROUTES.admin.users.base, {
  ...
  useEffect(() => {
    function handleOutside(e: MouseEvent) {
      if (
        agentComboRef.current &&
        !agentComboRef.current.contains(e.target as Node)
      ) {
        setAgentDropdownOpen(false);
      }
      if (
        tableComboRef.current &&
        !tableComboRef.current.contains(e.target as Node)
      ) {
  ...
  const confirmEvent = async (eventId: string) => {
    ...
    try {
      await toast.promise(
        axiosRequest.patch(API_ROUTES.network.events.update(eventId), {
          status: "CONFIRMED",
        }),
        {
  ...
  const handleRemit = async () => {
    setIsRemitting(true);
    try {
      await toast.promise(axiosRequest.post(API_ROUTES.network.remit), {
        loading: "Running remittance job...",
        success: "Remittance job completed successfully",
        error: (err) =>
          err?.response?.data?.detail ||
          err?.response?.data?.message ||
          "Remittance job failed",
      });
      fetchEvents();
    } catch {
      // handled by toast.promise
  ...
  const handleAdjustSubmit = async () => {
    ...
    try {
      await toast.promise(
        axiosRequest.post(
          API_ROUTES.network.agents.adjust(adjustAgentId.trim()),
          {
            points: adjustPoints,
            reason: adjustReason,
  ...
  const getActionButtons = (event: NetworkEvent): ActionButton[] => [
    ...
    },
    {
      ...
      onClick: () => {
        setSelectedRow(null);
        router.push(
          `${PAGE_ROUTES.dashboard.network.events.details(event.id)}?edit=true`,
        );
      },
    },
    {
          ...
          <div className="flex justify-between items-center gap-4 flex-wrap mb-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Monitor and manage all agent activity events
              </p>
            </div>
                ...
                <ul className="absolute z-10 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                  {agents
                    .filter((a) =>
                      agentLabel(a)
                        .toLowerCase()
                        .includes(tableAgentSearch.toLowerCase()),
                    )
                    .map((a) => (
                      <li
                        key={a.id}
                  ...
                  {agents.filter((a) =>
                    ...
                    <li className="px-3 py-2.5 text-sm text-gray-400 italic">
                      No agents found
                    </li>
                  )}
                </ul>
                ...
                {events.map((event, index) => {
                  ...
                  return (
                    <tr
                      ...
                      onClick={() =>
                        router.push(
                          PAGE_ROUTES.dashboard.network.events.details(
                            event.id,
                          ),
                        )
                      }
                    >
                      ...
                      <td className="px-6 py-4">
                        {(() => {
                          ...
                          const label =
                            name || (local ? agentFullName(local) : "");
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${event.is_remitted ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-600"}`}
                        >
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
            <div className="p-6">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                Run Point Remittance?
              </h3>
              <p className="text-sm text-gray-500 leading-relaxed">
                This will process all unremitted confirmed events and credit the
                naira equivalent of each agent&apos;s awarded points directly to
                their wallet. Only events that have not yet been remitted will
                be processed.
              </p>
              <p className="text-xs text-amber-600 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2 mt-3">
                This action cannot be undone. Ensure all events have been
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
            <div className="p-6">
              ...
              <p className="text-sm text-gray-500">
                This will mark the event as confirmed and award the associated
                points to the agent. This action cannot be undone.
              </p>
            </div>
            <div className="flex justify-end items-center gap-3 px-6 py-4 border-t border-gray-100">
              <button
      ...
      {showAdjustModal && (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between p-6 border-b border-gray-100 shrink-0">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  Award or deduct points for an agent
                </p>
              </div>
              <button
                onClick={closeAdjustModal}
            ...
            <div className="p-6 space-y-5 overflow-y-auto overflow-x-hidden min-h-0">
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm text-blue-700">
                Manually credit or debit points on an agent&apos;s network
                balance. Use a{" "}
                <span className="font-semibold">positive value</span> to award
                points and a{" "}
                <span className="font-semibold">negative value</span> to deduct
                them. This action is logged and cannot be undone.
              </div>
              ...
              <div className="space-y-1">
                ...
                />
                <p className="text-xs text-gray-400">
                  Must belong to the selected agent
                </p>
              </div>
              <div className="space-y-1">
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
            ...
            <div className="flex justify-end items-center gap-3 px-6 py-4 border-t border-gray-100 shrink-0">
              ...
              <button
                onClick={() => {
                  if (!adjustAgentId.trim()) {
                    toast.error(MESSAGES.MSG_AGENT_IS_REQUIRED);
                    return;
                  }
                  if (!adjustPoints || adjustPoints === 0) {
                    toast.error(MESSAGES.MSG_POINTS_MUST_BE_A_NONZERO_NUMBER);
          ...
          >
            <div className="p-6">
              ...
              <p className="text-sm text-gray-500 leading-relaxed">
                ...
                <span
                  className={`font-semibold ${adjustPoints >= 0 ? "text-green-600" : "text-red-600"}`}
                >
                  {adjustPoints >= 0 ? "+" : ""}
                  {adjustPoints} point
                </span>{" "}
