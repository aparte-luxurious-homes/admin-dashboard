...
const AgentVerificationQueueCard = () => {
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const response = await axiosRequest.get(API_ROUTES.verifications.myQueue, {
            ...
            return {
                ...
                latestPropertyName: body?.items?.[0]?.property?.name ?? body?.data?.[0]?.property?.name ?? null,
            };
        },
        staleTime: FIVE_MIN,
                    ...
                    <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                            <div className="flex items-baseline gap-2">
                                <span className={`text-3xl font-bold ${pending === 0 ? "text-gray-300" : "text-gray-800"}`}>
                                    {pending}
                                </span>
                            ...
                            {pending > 0 && data?.latestPropertyName ? (
                                ...
                                <p className="text-xs text-gray-400 mt-1">All clear — no properties waiting</p>
                            ) : null}
                        </div>
                        <Icon
                            icon={pending === 0 ? "solar:check-circle-bold-duotone" : "mdi:chevron-right"}
