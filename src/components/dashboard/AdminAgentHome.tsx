...
/**
 * The admin and agent home. Owners have their own page (OwnerHome) and never
 * render this one, which is why the dashboard route loads this module on
 * demand: it carries the charts and the MUI grid, about 70KB an owner would
 * otherwise download to look at a calendar.
 */
...
const AdminAgentHome = () => {
  const { user } = useAuth();
  const { isAdmin, isAgent, canViewGatewayBalances } = usePermissions();
  ...
  const fetchWallet = useCallback(async () => {
    ...
    try {
      ...
      const ngnWallet =
        wallets.find((w: Wallet) => w.currency === "NGN") || wallets[0];
  ...
  const fetchMentorship = useCallback(async () => {
    ...
    try {
      ...
      const items =
        body?.items ?? body?.data ?? (Array.isArray(body) ? body : []);
  ...
  const propertyChartLabels =
    stats?.properties?.map((item) => item?.month?.slice(0, 3) ?? "") || [];
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Grid container spacing={2}>
                    ...
                    <div className="p-5 h-[270px] border border-gray-200 rounded-2xl bg-white shadow-sm">
                      {isStatLoading ? (
                        ...
                        <div>
                          ...
                          <LineChart
                            labels={propertyChartLabels}
                            datasets={propertyChartDatasets}
                          />
                        </div>
                      ) : (
                        <div className="flex flex-col items-center justify-center h-full gap-2 text-gray-400">
                  ...
                  <Grid size={{ xs: 12, md: 4 }}>
                    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-5 h-full">
                      ...
                      {mentorshipLoading ? (
                        ...
                        (() => {
                          const isMentor =
                            currentMentorship.mentor_id === String(user?.id);
                          const partner = isMentor
                            ? currentMentorship.mentee
                            : currentMentorship.mentor;
                          const partnerTier = isMentor
                            ? currentMentorship.mentee_tier
                            : currentMentorship.mentor_tier;
                          const roleLabel = isMentor
                            ? "Your Mentee"
                            : "Your Mentor";
                          const partnerName =
                            [
                              ...
                              partner?.last_name || partner?.profile?.last_name,
                            ]
                              .filter(Boolean)
                              .join(" ") ||
                            partner?.email ||
                            "—";
                          const sc =
                            MENTORSHIP_STATUS_CONFIG[
                              currentMentorship.status
                            ] ?? MENTORSHIP_STATUS_CONFIG.ENDED;
                          return (
                            <div className="bg-gray-50 rounded-xl p-3 space-y-2.5">
                              <div className="flex items-center justify-between">
                                ...
                                <span
                                  className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold ${sc.bg} ${sc.text}`}
                                >
                                  {currentMentorship.status}
                                </span>
                              </div>
                              <div className="flex items-center gap-2.5">
                                ...
                                <div className="min-w-0">
                                  ...
                                  {partnerTier && (
                                    <p
                                      className={`text-[10px] font-semibold ${TIER_COLORS[partnerTier] ?? "text-gray-500"}`}
                                    >
                                      {partnerTier.charAt(0) +
                                        partnerTier.slice(1).toLowerCase()}
                                    </p>
                                  )}
                                </div>
                              </div>
        ...
        {/* Sidebar — admin only; owners have their own home */}
        {showSidebar && (
