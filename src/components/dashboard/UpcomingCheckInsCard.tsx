        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden h-full">
            ...
            {isLoading ? (
                ...
                <div className="px-5 py-10 flex flex-col items-center justify-center text-center gap-2">
                    <Icon icon="solar:calendar-mark-line-duotone" className="w-10 h-10 text-gray-300" />
                    <p className="text-sm text-gray-500">No bookings in the next 30 days</p>
                </div>
            ) : (
                <ul className="divide-y divide-gray-100">
                    {items.map((b) => {
                        ...
                        const dayLabel =
                            days === 0 ? "Today" : days === 1 ? "Tomorrow" : days < 0 ? "Past" : `In ${days}d`;
                        return (
                            <li key={b.id}>
                                <Link
                                    href={PAGE_ROUTES.dashboard.bookingManagement.bookings.details(b.id)}
