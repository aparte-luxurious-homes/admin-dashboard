...
export default function PlatformConflictsPage() {
  ...
  const { data: conflictsData, isLoading } = GetConflicts(
    "UNRESOLVED",
    page,
    20,
  );
      ...
      showAlert({
        ...
        onConfirm: () => {
          resolveConflict(conflictId, {
            onSuccess: () =>
              toast.success(MESSAGES.MSG_CONFLICT_MARKED_AS_RESOLVED),
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-600 mt-1">
            Review and resolve double-bookings detected by iCal Sync.
          </p>
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="p-12 text-center text-zinc-500">
            <Icon
              ...
              className="text-5xl mx-auto mb-4 opacity-20 text-green-500"
            />
              ...
              <tbody className="divide-y divide-zinc-100">
                {conflicts.map((conflict: any) => (
                  <tr key={conflict.id} className="hover:bg-zinc-50">
                    ...
                    <td
                      className="px-6 py-4 text-zinc-500 max-w-[150px] truncate"
                      title={conflict.external_uid}
                    >
