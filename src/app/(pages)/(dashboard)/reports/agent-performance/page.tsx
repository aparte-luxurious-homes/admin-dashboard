...
const downloadCsv = (filename: string, rows: AgentPerformanceRow[]) => {
  ...
  const valueFor = (r: AgentPerformanceRow, h: string): unknown => {
    ...
    switch (h) {
      case "assigned_count_week":
        return r.assigned_bookings.count_week;
      case "assigned_count_mtd":
        return r.assigned_bookings.count_mtd;
      case "assigned_count_total":
        return r.assigned_bookings.count_total;
      case "assigned_gmv_total":
        return r.assigned_bookings.gmv_total;
      case "assigned_last_booking_at":
        return r.assigned_bookings.last_booking_at;
      case "referred_count_week":
        return r.referred_bookings.count_week;
      case "referred_count_mtd":
        return r.referred_bookings.count_mtd;
      case "referred_count_total":
        return r.referred_bookings.count_total;
      case "referred_gmv_total":
        return r.referred_bookings.gmv_total;
      case "referred_last_booking_at":
        return r.referred_bookings.last_booking_at;
      default:
        return rec[h];
    }
  };
...
const AgentPerformanceReportPage = () => {
  ...
  const handleExport = () => {
    ...
    toast.success(
      `Exported ${filtered.length} row${filtered.length === 1 ? "" : "s"}`,
    );
  };
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Five-numbers framework — listings, verified listings, MTD, lifetime,
            and the active-agent flag. Drives the dashboard's Top Agents widget.
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-4 flex flex-wrap items-center gap-3">
        ...
        <span className="text-xs text-gray-500 ml-auto">
          {filtered.length} of {agentsRaw?.length ?? 0} agent
          {(agentsRaw?.length ?? 0) === 1 ? "" : "s"}
        </span>
      </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right tabular-nums text-gray-700">
                        ...
                        <span className="text-[11px] text-gray-400">
                          {" "}
                          / {r.assigned_bookings.count_mtd} /{" "}
                          {r.assigned_bookings.count_total}
                        </span>
                      ...
                      <td className="px-4 py-3 text-right tabular-nums text-gray-700">
                        ...
                        <span className="text-[11px] text-gray-400">
                          {" "}
                          / {r.referred_bookings.count_mtd} /{" "}
                          {r.referred_bookings.count_total}
                        </span>
                        <div className="text-[10px] text-gray-400 font-normal">
                      ...
                      <td className="px-4 py-3 text-[11px] text-gray-500">
                        {r.last_listed_at ? (
                          <span
                            className={
                              isDormant ? "text-red-600 font-medium" : ""
                            }
                          >
  ...
  >
    <div
      className={`text-[10px] font-semibold uppercase tracking-wider ${
        emphasis ? "text-white/80" : "text-gray-500"
      }`}
    >
      {label}
    </div>
    <div
      className={`text-2xl font-bold mt-0.5 ${emphasis ? "text-white" : "text-gray-900"}`}
    >
      {value}
    </div>
    <div
      className={`text-[11px] mt-0.5 ${emphasis ? "text-white/70" : "text-gray-400"}`}
    >
      {hint}
    </div>
  </div>
