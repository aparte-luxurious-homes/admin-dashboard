...
// phase 2 — zone assignments not included
  ...
  const fetchNetworkSummary = useCallback(async () => {
    ...
    try {
      const [meRes] = await Promise.allSettled([
        ...
        // axiosRequest.get(API_ROUTES.network.myZoneAssignments), // phase 2
      ]);
      if (meRes.status === "fulfilled") setNetworkSummary(meRes.value?.data?.data ?? null);
      // if (zoneRes.status === "fulfilled") { // phase 2
      //   const body = zoneRes.value?.data?.data;
      //   const assignments = body?.data ?? (Array.isArray(body) ? body : []);
